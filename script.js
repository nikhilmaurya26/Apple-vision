...
fetch('https://65732104192318b7db419a31.mockapi.io/apple-clone')
  .then(response => response.json())
  .then(data => {
    const navLinks = data[0].navLinks;
    const page6Data = data[1].page6; 
    console.log(page6Data);
    const navContainer = document.getElementById('myTopnav');
    const searchIcon = document.querySelector('.nav-top img[src*="icons8-search"]');
    
    navLinks.forEach(link => {
      const anchor = document.createElement('a');
      anchor.textContent = link; 
      navContainer.insertBefore(anchor, searchIcon);
    });

    populatePage6(page6Data);
  })
  .catch(error => {
    console.error('Error fetching data:', error);
  });

function populatePage6(data) {
  console.log(data.content)
    ...
    page6Element.innerHTML = '';
  
    // Append elements to #page6
    page6Element.appendChild(h3);
    page6Element.appendChild(h1);
    page6Element.appendChild(p);
}
  

function myFunction() {
  var x = document.getElementById("myTopnav");
  var navTop = document.querySelector("#page > nav > .nav-top");
  var pageBottom = document.querySelector(".page-bottom");
